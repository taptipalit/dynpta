...
bool ConstraintGraph::steensgardMoveInEdgesToRepNode(ConstraintNode* node, ConstraintNode* rep ) {
    std::vector<ConstraintEdge*> edgesList;
    ...
    while(!edgesList.empty()) {
        ConstraintEdge* edge = edgesList.back();
        edgesList.pop_back();
	ConstraintNode* srcNode = edge->getSrcNode();
        if(LoadCGEdge* load = dyn_cast<LoadCGEdge>(edge)) {
            //removeLoadEdge(load);
            //addLoadCGEdge(srcId,newDstNodeID);
	   if (srcNode->getId() == rep->getId())
                removeLoadEdge(load);
    	   else if(!(hasEdge(srcNode,rep,ConstraintEdge::Load))){
		node->removeIncomingLoadEdge(load);
		LoadCGEdgeSet.erase(load);
		srcNode->removeOutgoingLoadEdge(load);
		load->setNewDstNode(rep);
    		rep->addIncomingLoadEdge(load);
		srcNode->addOutgoingLoadEdge(load);
		LoadCGEdgeSet.insert(load).second;		
	   }
	   else 
		removeLoadEdge(load);
	    ...
	    if (srcNode->getId() == rep->getId())
                removeStoreEdge(store);		
	    else if(!(hasEdge(srcNode,rep,ConstraintEdge::Store))){
		node->removeIncomingStoreEdge(store);
		StoreCGEdgeSet.erase(store);
		srcNode->removeOutgoingStoreEdge(store);
                store->setNewDstNode(rep);
                rep->addIncomingStoreEdge(store);
		srcNode->addOutgoingStoreEdge(store); 
		StoreCGEdgeSet.insert(store).second;
	    ...
	    if (srcNode->getId() == rep->getId())
                removeDirectEdge(copy);
	    else if(!(hasEdge(srcNode,rep,ConstraintEdge::Copy))){
		node->removeIncomingDirectEdge(copy);
		directEdgeSet.erase(copy);
		srcNode->removeOutgoingDirectEdge(copy);
                copy->setNewDstNode(rep);
                rep->addIncomingDirectEdge(copy);
		srcNode->addOutgoingDirectEdge(copy);
		directEdgeSet.insert(copy).second;
	    }
	    else
		removeDirectEdge(copy); 
...
bool ConstraintGraph::steensgardMoveOutEdgesToRepNode(ConstraintNode* node, ConstraintNode* rep ) {
        std::vector<ConstraintEdge*> edgesList;
        ...
        while(!edgesList.empty()) {
                ConstraintEdge* edge = edgesList.back();
           	edgesList.pop_back();
		ConstraintNode* dstNode = edge->getDstNode();
                if(LoadCGEdge* load = dyn_cast<LoadCGEdge>(edge)) {
                        //emoveLoadEdge(load);
                        //addLoadCGEdge(newSrcNodeID,dstId);
			if (dstNode->getId() == rep->getId())
                        	removeLoadEdge(load);
			else if(!(hasEdge(rep,dstNode,ConstraintEdge::Load))){
				node->removeOutgoingLoadEdge(load);
				LoadCGEdgeSet.erase(load);
				dstNode->removeIncomingLoadEdge(load);
                		load->setNewSrcNode(rep);
                		rep->addOutgoingLoadEdge(load);
				dstNode->addIncomingLoadEdge(load);
				LoadCGEdgeSet.insert(load).second;
			}
			else
				removeLoadEdge(load);
			...
			if (dstNode->getId() == rep->getId())
                        	removeStoreEdge(store);
			else if(!(hasEdge(rep,dstNode,ConstraintEdge::Store))){
				node->removeOutgoingStoreEdge(store);
				StoreCGEdgeSet.erase(store);
				dstNode->removeIncomingStoreEdge(store);
                                store->setNewSrcNode(rep);
                                rep->addOutgoingStoreEdge(store);
				dstNode->addIncomingStoreEdge(store);
				StoreCGEdgeSet.insert(store).second;
                        }
			else 
				removeStoreEdge(store);
			...
			if (dstNode->getId() == rep->getId())
                        	removeDirectEdge(copy);
			else if(!(hasEdge(rep,dstNode,ConstraintEdge::Copy))){
				node->removeOutgoingDirectEdge(copy);
				dstNode->removeIncomingDirectEdge(copy);
				directEdgeSet.erase(copy);
                                copy->setNewSrcNode(rep);
                                rep->addOutgoingDirectEdge(copy);
				dstNode->addIncomingDirectEdge(copy);
				directEdgeSet.insert(copy).second;
                        }
			else
				removeDirectEdge(copy);
...
void ConstraintGraph::dump(StringRef fileName) {
    GraphPrinter::WriteGraphToFile(llvm::outs(), fileName, this);
}

void ConstraintGraph::dumpInitial() {
    if (ConsCGDotGraph)
        GraphPrinter::WriteGraphToFile(llvm::outs(), "consCG_initial", this);
}
