...
$LLVMROOT/opt -encryption -steens-fast -skip-csa=false -skip-vfa=false -optimized-check=true -callsite-threshold=10 -confidentiality=true -partitioning=true -hoist-taint-checks=true  $filewithlibc.bc -o $fileinst.bc
...
./run_glibc.sh $filedfsan.o $file "-lcrypt"
