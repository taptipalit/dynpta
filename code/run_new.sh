...
$LLVMROOT/opt -encryption -DFSAN=true -debug-only=encryption $file.bc -o $fileinst.bc
$LLVMROOT/opt --dfsan $fileinst.bc -o $fileinst.bc 
...
$LLVMROOT/clang  $GGDB -fsanitize=dataflow aes_inmemkey.s aes_helper.c $fileinst.o -o $file
if [ $? -ne 0 ]
then
    exit 1
fi
