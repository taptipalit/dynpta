...
$LLVMROOT/opt -encryption -steens-fast -skip-csa=false -skip-vfa=false -optimized-check=true -confidentiality=true -partitioning=true -hoist-taint-checks=true $filewithlibc.bc -o $fileinst.bc
if [ $? -ne 0 ]
then
    exit 1
fi
